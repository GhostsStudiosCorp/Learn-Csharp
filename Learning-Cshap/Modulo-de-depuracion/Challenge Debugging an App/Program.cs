...
Simular transacciones: las instrucciones de nivel superior configuran los datos de la aplicación y 
simulan una serie de transacciones mediante una pequeña matriz testData o un mayor número de transacciones 
generadas aleatoriamente.

Inicializar la caja: se utiliza el método LoadTillEachMorning para configurar la caja registradora con un 
número predefinido de billetes de cada denominación.

Procesar transacciones: se usa el método MakeChange para administrar la caja durante las transacciones de compra.

Informar del estado de la caja: se utiliza el método LogTillStatus para mostrar el número de billetes de cada 
denominación que hay actualmente en la caja.

Informar del saldo: se utiliza el método TillAmountSummary para mostrar un mensaje que indica la 
cantidad de efectivo en la caja.
*/
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens, int fives, int ones = 0)
{
    cashTill[3] += twenties;
    cashTill[2] += tens;
    cashTill[1] += fives;
